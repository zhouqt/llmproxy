...
mod tests {
    ...
    /// wiremock's `header()` exact matcher splits header values on commas
    /// (treating `,` as the HTTP multi-value separator), which breaks any
    /// UA string like `claude-cli/2.1.224 (external, claude-vscode,
    /// agent-sdk/0.3.224)` — the value would be split into three
    /// fragments and never equal the full string. Compare the raw value
    /// verbatim instead.
    struct HeaderVerbatim(&'static str, &'static str);

    impl Match for HeaderVerbatim {
    ...
    async fn default_user_agent_simulates_claude_code() {
        ...
        // config, outbound requests must carry the `claude-cli/<version>
        // (...)` User-Agent so LLM providers classify the proxy's
